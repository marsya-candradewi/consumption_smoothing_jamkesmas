use "`RAW_HEALTH4'/b3b_rj1.dta" , clear
collapse (sum) rj02, by (hhid07 pidlink)

mmerge hhid07 pidlink using "`RAW_HEALTH4'/b3b_rj0.dta"
...
mmerge hhid07 pidlink using "`RAW_ID4'/bk_ar1.dta"
...
preserve
	*head health facil util*
	...
	drop if pairhh==1
	keep hhid07 houtpatient hnumvisit
save "`WORKING_HEALTH'/hhealthutil07.dta", replace
restore
	...
	*spouse health facil util*
	...
	duplicates tag hhid07, gen(pairhh)
	drop if pairhh==1
	keep hhid07 soutpatient snumvisit
	save "`WORKING_HEALTH'/shealthutil07.dta", replace
