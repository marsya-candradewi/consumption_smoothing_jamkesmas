...
foreach s of local shocks {
	...
	* (4) ΔC ~ ΔH + controls + kab FE + provid FE
    eststo reg_`s'_FEprov: reghdfe d_lpce d_`s' base_heduc base_headage base_headagesq ///
        d_prodage d_child d_fhead d_urban moved ///
		if t == 1, absorb(provid14) vce(cluster provid14)
...
* Panel A: ADL
* -------------------------------- *
esttab reg_headADL_nocontrol reg_headADL_controls reg_headADL_FEprov ///
...
* -------------------------------- *
esttab reg_dh_nocontrol reg_dh_controls reg_dh_FEprov using "`TABLES'/panel_totcons_Days.tex", ///
...
* Panel B: Days of Sickness
* -------------------------------- *
esttab reg_dh_nocontrol reg_dh_controls reg_dh_FEprov using "`TABLES'/panel_med_Days.tex", ///
    replace label nonotes noobs nodepvar ///
    b(%9.3f) se(%9.3f)     prehead("") posthead("") ///
    keep(d_dh) nomtitles nonumber ///
	star(* 0.10 ** 0.05 *** 0.01) ///
...
* Panel B: Days of Sickness
* -------------------------------- *
esttab reg_dh_nocontrol reg_dh_controls reg_dh_FEprov using "`TABLES'/panel_earn_Days.tex", ///
    replace label nonotes noobs nodepvar ///
    b(%9.3f) se(%9.3f)     prehead("") posthead("") ///
    keep(d_dh) nomtitles nonumber ///
	star(* 0.10 ** 0.05 *** 0.01) ///
